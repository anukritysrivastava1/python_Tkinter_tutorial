...
class Contacts:
    ...
    def __init__(self, root):
        ...
        ttk.style=ttk.Style()
        ttk.style.configure("Treeview", font=('robotomono', 10))
        ttk.style.configure("Treeview.Heading", font=("robotomono",12, 'bold'))
    ...
    def create_left_icon(self):
        photo = PhotoImage(file='logo img/logo-contact.png')
        label = Label(image=photo)
        label.image=photo
        label.grid(row=0,column=0)

    def create_label_frame(self):
        labelframe=LabelFrame(self.root, text='Create New Contacts', bg="orange", font="robotomono 10")
        labelframe.grid(row=0, column=1, padx=8, pady=8, sticky='ew')
        Label(labelframe, text='Name: ', bg='black', fg="white").grid(row=1, column=1, sticky=W, pady=2, padx=15)
        self.namefield=Entry(labelframe)
        self.namefield.grid(row=1, column=2, sticky=W, padx=5, pady=2)
        Label(labelframe, text='Email: ', bg="black", fg='white').grid(row=2, column=1, sticky=W, pady=2, padx=15)
        ...
        Button(labelframe, text='Add Contact', command=self.on_add_contact_button_clicked, bg="Yellow", fg="black").grid(row=4, column=2, sticky=E, padx=5, pady=5)
    ...
    def create_buttom_buttons(self):
        ...
        Button(text='Modify Selected', command=self.on_modify_selected_button_clicked, bg='blue', fg='white').grid(row=8,column=2,sticky=W)
    ...
    def add_new_contact(self):
        if self.new_contacts_validated():
            ...
            self.message['text']='New Conatact {} added'.format(self.namefield.get())
    ...
    def new_contacts_validated(self):
        return len(self.namefield.get()) != 0 and len(self.emailfield.get()) !=0 and len(self.namefield.get())!=0
